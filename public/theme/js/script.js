(function($) { 
	...
	if($('.side-nav').length > 0){
		...
		if (!$('body').hasClass('one-page')){
			...
			$('<a href="#" class="menuBtn"><i class="fa fa-bars"></i></a><div class="responsive-nav"></div>').prependTo('body');
			$('.responsive-nav').html('<h3>E-İhracat Menü</h3><ul>'+men+'</ul>').find('ul').removeAttr('class');;
			if($('html').css('direction') == 'rtl'){
				$('.responsive-nav h3').text('');
			}
			$('.menuBtn').click(function(e){
	...
	function toggleFields() {
		if ($("#serviceCategorySelect").val() === "appService1"){
			$("#hukuklojistik").show();
			$("#ulusal").hide();
			$("#uluslararası").hide();
		}else if ($("#serviceCategorySelect").val() === "appService2"){
			$("#ulusal").show();
			$("#hukuklojistik").hide();
			$("#uluslararası").hide();
		}else if ($("#serviceCategorySelect").val() === "appService3"){
			$("#uluslararası").show();uluslararası
			$("#hukuklojistik").hide();
			$("#ulusal").hide();
		}
