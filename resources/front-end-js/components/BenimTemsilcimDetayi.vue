<template>
    <div class="exportRepresentiveInfos">
        <div class="blockHeader">
            <h1>
                {{ country_detail.nameTr }} <br />
                <small>Ülke Temsilcileri</small>
            </h1>
        </div>

        <div
            class="representiveArea"
            v-for="(representer, index) in representers"
            :key="index"
        >
                ...
                <div class="representiveContact">
                    <ul>
                        ...
                        <li>
                            <span>Telefon:</span>
                            <p
                                v-for="(phone, index) in representer.phones"
                                :key="index"
                            >
                                {{ phone }},
                            </p>
                        ...
                        <li>
                            <span>Faks:</span>
                            <p
                                v-for="(fax, index) in representer.faxes"
                                :key="index"
                            >
                                {{ fax }},
                            </p>
                        ...
                        <li>
                            <span>E-Posta:</span>
                            <p
                                v-for="(email, index) in representer.emails"
                                :key="index"
                            >
                                {{ email }},
                            </p>
    ...
    methods: {
        fetchCodeDetails() {
            return new Promise((resolve, reject) => {
                axios
                    .get(
                        `https://api.ihracatradari.com/comtrade/getcontacts?c=${this.$route.query.country}`
                    )
                    .then(response => {
        ...
        fetchCountryDetail() {
            return new Promise((resolve, reject) => {
                axios
                    .get(
                        `https://api.ihracatradari.com/comtrade/getcountry?countryName=${this.$route.query.country}`
                    )
                    .then(response => {
                        console.log(response.data);
